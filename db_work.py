...
def select(db_config: dict, sql: str) -> Tuple[Tuple, List[str]]:
    ...
    result = tuple()
    schema = []
    with DBContextManager(db_config) as cursor: # cursor - объект класса подклюния к БД
        if cursor is None:
            raise ValueError('Cursor not found')
        cursor.execute(sql)
        schema = [column[0] for column in cursor.description]
        result = cursor.fetchall() # Кортеж с результатом запроса
...
def execute_(dbconfig: dict, _sql:str):
    with DBContextManager(dbconfig) as cursor:
        if cursor is None:
            raise ValueError('Курсор не создан')
        result=cursor.execute(_sql)
